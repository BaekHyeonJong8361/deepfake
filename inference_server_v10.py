import os, io, time, uuid, asyncio, base64, tempfile, shutil, sys
...
# T2V module integration
_T2V_MODULE_PATH = '/home/t26106/ai-detection/project-g/src'
if _T2V_MODULE_PATH not in sys.path:
    sys.path.insert(0, _T2V_MODULE_PATH)
try:
    from ai_video_detector.server import get_t2v_analyzer, get_xai_output_dir as _get_t2v_xai_dir
    _T2V_AVAILABLE = True
    print("[v10] T2V module loaded successfully")
except Exception as _t2v_import_err:
    _T2V_AVAILABLE = False
    print(f"[v10] T2V module not available: {_t2v_import_err}")
...
async def run_t2v_analysis(video_id: str, url: str):
    """T2V 전용 분석: ai_video_detector.server.ModelAnalyzer 사용."""
    analysis_tasks[video_id]["status"] = "analyzing"
    try:
        # 로컬 /files/ 경로는 localhost URL로 변환
        if url.startswith('/files/'):
            url = f"http://localhost:{PORT}{url}"

        ana = get_t2v_analyzer()
        loop = asyncio.get_event_loop()
        resp = await loop.run_in_executor(
            None, lambda: ana.analyze_url(url, request_id=video_id, return_xai=True)
        )

        heatmap_url = ""
        if resp.xai_visualization and resp.xai_visualization.heatmaps:
            hm = resp.xai_visualization.heatmaps[0]
            heatmap_url = hm.overlay_url or hm.heatmap_url or ""

        forensic_text = resp.forensic_report.summary if resp.forensic_report else ""
        frame_probs = [float(fi) for fi in (resp.evidence.frame_importance or [])]

        result = {
            "decision": resp.decision,
            "score": resp.t2v_prob,
            "threshold": 0.5,
            "evidence": resp.evidence.model_dump(),
            "final_verdict": resp.decision,
            "deepfake_score": resp.t2v_prob * 100,
            "t2v_score": resp.t2v_prob * 100,
            "analysis_type": "T2V",
            "engine_label": "T2V (Text-to-Video Detection)",
            "xai_heatmap_url": heatmap_url,
            "xai_text": forensic_text,
            "suspicious_frames": [
                {"frameIndex": i, "probability": float(p), "time": f"{i/25:.2f}s"}
            ...
            "freq_contribution": 0,
            "top_regions": [],
            "forensic_report": forensic_text,
            "raw": {
                "score": resp.t2v_prob,
                "threshold": 0.5,
                "t2v_evidence": resp.model_dump(),
            },
        }
        print(f"[T2V] RESULT: verdict={resp.decision}, t2v_prob={resp.t2v_prob:.4f}, heatmap={'yes' if heatmap_url else 'no'}")
        analysis_tasks[video_id]["result"] = result
        analysis_tasks[video_id]["status"] = "completed"
    except Exception as e:
        import traceback
...
if _T2V_AVAILABLE:
    try:
        _xai_dir = _get_t2v_xai_dir()
        _xai_dir.mkdir(parents=True, exist_ok=True)
        app.mount("/xai", StaticFiles(directory=str(_xai_dir)), name="xai")
        print(f"[v10] /xai mounted at {_xai_dir}")
    except Exception as _xai_err:
        print(f"[v10] /xai mount failed: {_xai_err}")
...
async def t2v_analyze(request: Request):
    if not _T2V_AVAILABLE:
        return await deepfake_analyze(request)
    data = await request.json()
    print(f"[T2V] Request body keys: {list(data.keys())}")
    url = data.get("url") or data.get("videoUrl") or data.get("video_url") or data.get("s3_url", "")
    if not url:
        return {"status": "failed", "message": "url is required"}
    video_id = "t2v_" + uuid.uuid4().hex[:8]
    try:
        ana = get_t2v_analyzer()
        loop = asyncio.get_event_loop()
        resp = await loop.run_in_executor(
        ...
        print(f"[T2V] RESULT: decision={resp.decision}, t2v_prob={resp.t2v_prob:.4f}, model={resp.model_used}")
        return result
    except Exception as e:
...
async def video_url(request: Request, bg_tasks: BackgroundTasks):
    data = await request.json()
    url = data.get("url", "")
    analysis_type = data.get("type", "SHEN")
    video_id = uuid.uuid4().hex[:12]
    analysis_tasks[video_id] = {"status": "pending"}

    if analysis_type == "T2V" and _T2V_AVAILABLE:
        bg_tasks.add_task(run_t2v_analysis, video_id, url)
    ...
    bg_tasks.add_task(run_analysis, video_id, local_path, analysis_type)
    return {"video_id": video_id}
@app.get("/api/xai-image")
async def xai_image_proxy(path: str):
    """T2V heatmap 파일을 base64 data URL로 반환 — 브라우저가 ceprj 포트에 직접 접근 불가능한 경우 Spring Boot가 이 엔드포인트를 통해 이미지를 가져감."""
    if not path.startswith("/xai/"):
        raise HTTPException(status_code=400, detail="Invalid path")
    if ".." in path:
        raise HTTPException(status_code=400, detail="Invalid path")
    xai_root = _get_t2v_xai_dir() if _T2V_AVAILABLE else None
    if xai_root is None:
        raise HTTPException(status_code=503, detail="T2V not available")
    fpath = xai_root / path[5:]
    if not fpath.exists():
